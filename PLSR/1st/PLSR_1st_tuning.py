...
for n_comp in range(1, max_components + 1):
    print(f'当前进度为{n_comp}/{max_components+1}')
...
print(f'最优参数为{best_idx+1}，其RMSE为{rmse_list[best_idx]:.3f}')

plt.figure(figsize=(8,5))
plt.plot(range(1, max_components+1), rmse_list, marker='o')
plt.xlabel('Number of PLS components')
plt.ylabel('RMSE')
plt.title('PLS Regression Hyperparameter Tuning Curve')
plt.grid()
plt.show()
...
# ========== 在测试集上评估 ==========
best_model = PLSRegression(n_components=best_idx+1,scale=False, max_iter=10000)
...
test_summary = pd.DataFrame({
    ...
    'Value': [best_idx+1, val_rmse, val_r2, test_rmse, test_r2]
})
