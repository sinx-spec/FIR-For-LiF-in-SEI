...
print(f'最优参数为{alphas[best_idx]}，其RMSE为{rmse_list[best_idx]:.3f}')
...
plt.ylabel('RMSE')
plt.title('Ridge Regression Hyperparameter Tuning Curve')
plt.grid()
plt.show()
...
process.to_excel('1st_process.xlsx', index=False)
print("参数寻优过程已保存")

# ========== 在测试集上评估 ==========
best_model = Ridge(alpha=alphas[best_idx])
...
test_summary = pd.DataFrame({
    ...
    'Value': [alphas[best_idx], val_rmse, val_r2, test_rmse, test_r2]
})

with pd.ExcelWriter('1st_prediction_results.xlsx') as writer:
